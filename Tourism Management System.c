...
void CancelTicket(user *h)
{
    ...
    if(flag==0)
    {
        printf("Your ticket has been successfully cancelled\nA refund of $ %f for Tour Code %s for %d tickets will soon be made to your original source of purchase\n",h->price,h->place,h->numtick);
        strcpy(h->place,"N/A");
        h->price=0.0;
        h->numtick=0;
        WriteToFile(t);
