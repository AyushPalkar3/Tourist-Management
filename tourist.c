...
{
	system("CLS");
    printf("\tPRICE LIST\n=============================\n1. IND - Traditional INDIA -   Rs 50,000.00/-\n2. AUS - Kango AUSTRALIA -     Rs 75,000.00/-\n3. UAE - DUBAI Dhamaka -       Rs 43,000.00/-\n4. MDV - Waterry MALDIVES -    Rs 62,000.00/-\n"
           "5. TH  - Bang bang THAILAND -  Rs 31,000.00/-\n6. BA  - BALI Devotes -        Rs 48,000.00/-\n7. ENG - Cheessee ENGLAND -    Rs 56,000.00/-\n8. MX  - Chocolicius MEXICO-   Rs 61,000.00/-\n9. IT  - Italian ITALY -       Rs 42,000.00/-\n10.TR  - Global TURKEY -       Rs 54,000.00\n \t\t \n \t \t Note !\n Every package mentioned above is 1 week pakage.\n ");
}
...
void BookTicket(user *h)
{
    ...
    ShowBrochure();
    float pricelist[]={50000.0,75000.0,43000.0,62000.0,31000.0,48000.0,56000.0,61000.0,42000.0,54000.0};
    fflush(stdin);
    printf("\nEnter place code (eg: IND , ENG)\n");
    scanf(" %[^\n]s",place);
    char choice;
    fflush(stdin);
    ...
    if(choice!='1')
        return;
    if(strcmp(place,"IND")==0)
        price=pricelist[0];
    else if(strcmp(place,"AUS")==0)
        price=pricelist[1];
    else if(strcmp(place,"UAE")==0)
        price=pricelist[2];
    else if(strcmp(place,"MDV")==0)
        price=pricelist[3];
    else if(strcmp(place,"TH")==0)
        price=pricelist[4];
    else if(strcmp(place,"BA")==0)
        price=pricelist[5];
    else if(strcmp(place,"ENG")==0)
        price=pricelist[6];
    else if(strcmp(place,"MX")==0)
        price=pricelist[7];
    else if(strcmp(place,"IT")==0)
        price=pricelist[8];
    else if(strcmp(place,"TR")==0)
        price=pricelist[9];
    else
    {
...
void PrintTicket(user *h)
{
    ...
    if(fgetc(fp)==EOF)
    {
        fprintf(fp,"PARV Tourism Ticket\n===============\n\n");
    }
    fprintf(fp,"Email ID/Username: %s\nTour Code: %s\nTicket Cost: Rs %f/-\nNumber of tickets: %d\nTotal Cost: Rs %f/-\n",h->username,h->place,h->price,h->numtick,total);
    fclose(fp);
}
...
void CancelTicket(user *h)
{
    ...
    if(strcmp(h->place,"IND")==0)
        flag++;
    else if(strcmp(h->place,"AUS")==0)
        flag++;
    else if(strcmp(h->place,"UAE")==0)
        flag++;
    else if(strcmp(h->place,"MDV")==0)
        flag++;
    else if(strcmp(h->place,"TH")==0)
        flag++;
    else if(strcmp(h->place,"BA")==0)
        flag++;
    else if(strcmp(h->place,"ENG")==0)
        flag++;
    else if(strcmp(h->place,"MX")==0)
        flag++;
    else if(strcmp(h->place,"IT")==0)
        flag++;
    else if(strcmp(h->place,"TR")==0)
        flag++;
    else
    {
